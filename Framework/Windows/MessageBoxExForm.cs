...
namespace ConnectUO.Framework.Windows.Forms
{
	/// <summary>
    ...
    internal sealed class MessageBoxExForm : Form
	{
		...
		private void SetMessageSizeAndVisibility()
		{
			...
			{
				int maxWidth = _maxLayoutWidth/ 2;
...
//					break;
		...
		private void CenterForm()
		{
            if (Owner != null)
            {
                ...
                int x = ((Owner.Location.X + (Owner.Size.Width / 2)) - (Width / 2));
                int y = ((Owner.Location.Y + (Owner.Size.Height / 2)) - (Height / 2));
        ...
        private void SetResultAndClose(string result)
        {
            ...
            try
            {
                this.DialogResult = (DialogResult)Enum.Parse(typeof(DialogResult), result);
            }
            catch
            {
